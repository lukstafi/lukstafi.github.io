...
4. **The quantifier prefix supplies a matching-like mechanism that the earlier rounds overlooked** (§7): with a $\forall\exists$ prefix, branch premises can pin answer parameters to branch-specific terms over universal variables — exactly the mechanism of the thesis's GADT `Pair` example. The round-7 verdict "semi-unification is structurally incompatible with JCAQP" is not justified for nontrivial prefixes.
...
**Counterexample 4.1.** Signature $\{f/1, a\}$; instance variables $x, y$; single branch $D: x \doteq y$, $C: x \doteq y$ (or any trivial conclusion — the point is consistency). Candidate answers $A_N: x \doteq f^N(z) \land y \doteq f^N(w)$ and $A'_N: x \doteq f^N(a) \land y \doteq f^{N+1}(a)$. Here $k = 2$, $d = 0$, $n = 2$, so the claimed reach bound is $O(1)$, independent of $N$. But consistency of $D \land A$ requires unifying $x$'s and $y$'s images in full: $A_N$ is consistent with $D$, $A'_N$ is not, and the two differ only at depth $N$. Every position of the answer's terms affects satisfaction. The lemma's proof step "unification with depth-$d$ terms can reach at most depth $d$ into each substituted term" is false: unifying two *variables* merges their entire (unboundedly deep) bindings.
...
1. Rounds 1–7 analyzed the prefix almost exclusively through the no-escaping condition (as a *restriction*). The above shows it is also an *expressiveness* mechanism: universals + back-propagation = per-branch instantiation of the answer. The round-7 claim that JCAQP lacks any matching-like primitive is wrong for nontrivial prefixes.
...
- **Example 4.3**: the thesis's answer $\exists\gamma: y \doteq f(z,\gamma)$ verifies; a brute-force search over single-equation candidates additionally finds the parameter-free answers $y \doteq f(z,x)$, $y \doteq f(x,z)$, $y \doteq f(z,z)$ — all instances of the most general answer, as predicted.
...
- **Counterexample 4.1**: with $N = 200$, $A_N$ ($x \doteq f^{200}(z), y \doteq f^{200}(w)$) is an answer to the branch $x \doteq y \Rightarrow x \doteq y$ while $A'_N$ ($y \doteq f^{201}(a)$) is not — a difference at depth $200$ decided by an instance of depth $0$, refuting the round-7 reach bound concretely.
- **GADT `Pair`**: the expected answer verifies for relevance/consistency; the checker also confirms that the back-propagation-only alternative $\tau \doteq \mathrm{Term}((\alpha'',\beta''))$ passes relevance/consistency and is excluded only by the prefix conditions — consistent with §7's reading of where the prefix's power resides.
